...
--   * usuario_app       -> usuario_aplicacion   (UUID logico del end-user, JWT.sub)
--
-- El renaming es solo presentacional; la justificacion de la triple identidad
-- y de cada columna esta en schema/01_tables/18_audit_log.sql. La vista no
-- expone datos nuevos: solo etiquetas mas humanas para el frontend.
--
-- Acceso: SELECT a staff, authenticated (cliente final NO deberia leer el
-- audit completo; quien consume es el panel staff), service_role y quique.
-- La tabla audit_log subyacente esta protegida por trg_audit_log_append_only
-- y por las RLS pertinentes.
...
'R1/R3 Etapa 2: vista presentacional sobre audit_log. Traduce tipo_op (I/U/D) a texto y renombra usuario_db/rol_sesion/usuario_app a rol_efectivo/rol_postgres/usuario_aplicacion para consumo del panel staff.';
